...
List falldetected = [false];

class BackgroundCollectedPage extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final BackgroundCollectingTask task = BackgroundCollectingTask.of(
        context, rebuildOnChange: true);

    // Arguments shift is needed for timestamps as miliseconds in double could loose precision.
    final int argumentsShift = task.samples.first.timestamp
        .millisecondsSinceEpoch;

    final Duration showDuration = Duration(
        minutes: 2); // @TODO . show duration should be configurable
    ...
    final Duration argumentsStep = Duration(
        minutes: 1); // @TODO . step duration should be configurable
    ...
    DateTime beginningArgumentsStep = DateTime(
        beginningArguments.year, beginningArguments.month,
    ...
    var accl = lastSamples.map((sample) => sample.accl);
//    print(accl);
//    List falldetected = [false];
//    print(falldetected);
    int acclLen = accl.length;
    print(accl.last);
    if (accl.last >= 5.4) { // compare this value with a proper threshold
      if (accl.elementAt(acclLen - 2) >= 5.4 ){
        fallStr = "YES! :)";
        falldetected.add(true);
      }
    }
    else {
      if (!falldetected.contains(true)) fallStr = "No :(";
    }
      ...
      body: ListView(
        children: <Widget>[
          ...
          ListTile(
            ...
            title: const Text('Acceleration Data'),
            subtitle: const Text('In units'),
          ),
          LineChart(
