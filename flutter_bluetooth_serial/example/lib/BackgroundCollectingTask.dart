...
//  double waterpHlevel;
  ...
  DataSample({this.accl, this.timestamp});
}
  ...
  BackgroundCollectingTask._fromConnection(this._connection) {
    _connection.input.listen((data) {
      ...
      while (true) {
        ...
        int index = _buffer.indexOf('t'.codeUnitAt(0));    // send accl data after sending t
        if (index >= 0 && _buffer.length - index >= 2) {
          final DataSample sample = DataSample(
            accl: _buffer[index + 1] / 10,
...
//            waterpHlevel: (_buffer[index + 5] + _buffer[index + 6] / 100),
          ...
          //print("${sample.timestamp.toString()} -> ${sample.accl} / ${sample.temperature2}");
        }
        // Otherwise break
        else {
          break;
